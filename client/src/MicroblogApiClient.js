...
export default class MicroblogApiClient {
    ...
    async requestInternal(options) {
        let query = new URLSearchParams(options.query || {}).toString(); //renders object in proper query string format
        if(query !== ''){
            query = '?' + query;
        }
    ...
    async login(username, password){
        const response = await this.post('/tokens', null, {
            headers: {
                Authorization: 'Basic ' + btoa(username + ":" + password) //base64 encoding string
            }
        });
        if(!response.ok){
